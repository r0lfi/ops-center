"""
Vulnerability prioritization - a byte-for-byte duplicate of
app/services/priority.py, kept here because worker_ai's Docker build only
copies app/models, app/db, and app/core (not app/services - see
worker_ai/Dockerfile) and this is pure Python with no FastAPI dependency,
matching the project's existing convention of small duplicated files per
component (see metrics_tools.py's docstring for the same reasoning). Keep
