...
def _fake_db():
    ...
    db.execute.return_value.scalars.return_value.first.return_value = (
        None  # no existing matching action
    )
    db.execute.return_value.scalar_one_or_none.return_value = (
        None  # no host row needed for these assertions
    )
    return db
...
def test_request_approval_creates_a_pending_row_and_never_executes_anything():
    ...
    with patch("worker_ai.ai.actions.set_agent_state") as set_state:
        result = request_approval(
            ...
            _fake_agent(),
            "restart_service",
            {"hostname": "example-web-01", "service": "nginx"},
            "task-1",
            "because",
        )
    ...
    assert "godkjenn " + result["request_code"] in result["message"]
    assert "{action.request_code}" not in result["message"]
...
def test_reboot_host_is_level_three_and_high_risk():
    ...
    with patch("worker_ai.ai.actions.set_agent_state"):
        request_approval(
            db, _fake_agent(), "reboot_host", {"hostname": "example-web-01"}, None, "because"
        )
