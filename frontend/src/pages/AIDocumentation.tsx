...
export default function AIDocumentation() {
  ...
  const sections = documentation.filter((section) =>
    [section.title, ...section.paragraphs, ...(section.rows?.flat() ?? []), ...section.sources]
      .join(" ").toLocaleLowerCase("en").includes(normalized),
  );
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-semibold">Ops Center – technical documentation</h1>
        </div>
        <p className="max-w-4xl text-sm leading-6 text-muted-foreground">
          The platform, agent controls and configuration entry points. This is versioned documentation; your configured providers, permissions and live service status remain authoritative.
        </p>
        <div className="flex flex-wrap gap-4 text-sm text-primary">
          <Link className="underline underline-offset-4" to="/ai-agents/agents">Agent configuration</Link>
          <Link className="underline underline-offset-4" to="/ai-agents/settings">Model providers</Link>
          <Link className="underline underline-offset-4" to="/ai-agents/approvals">Approvals</Link>
          <Link className="underline underline-offset-4" to="/ai-agents/memory">History and memory</Link>
        </div>
      </div>
      <div className="relative max-w-xl">
        ...
        <Input id="documentation-search" type="search" className="pl-9" value={query}
          onChange={(event) => setQuery(event.target.value)} placeholder="Search runtime, memory, Docker, approval …" />
      </div>
      <p className="text-xs text-muted-foreground" role="status">{sections.length} of {documentation.length} chapters</p>
      <div className="grid items-start gap-6 lg:grid-cols-[230px_minmax(0,1fr)]">
        <nav aria-label="Documentation contents" className="rounded-lg border border-border p-4 lg:sticky lg:top-4">
        ...
        <div className="min-w-0 space-y-5">
          {sections.length === 0 && <p className="rounded-lg border border-border p-6 text-sm">No chapters match your search. Try another term or clear the search field.</p>}
          {sections.map((section) => (
            <Card key={section.id} id={section.id} className="scroll-mt-6">
              ...
              <CardContent className="space-y-4 text-sm leading-6">
                ...
                {section.flow && <ol aria-label="Data flow" className="grid gap-2 sm:grid-cols-2">
                ...
                <div className="border-t border-border pt-3 text-xs text-muted-foreground">
                  <p className="mb-1 font-medium">Project source references</p>
